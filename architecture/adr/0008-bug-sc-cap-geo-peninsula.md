...
**Estado:** Cerrado (2026-07-26). Bug confirmado con datos reales, corregido en producción y en el sistema local, histórico recalculado.
...
Impacto real: `mercado_sc_cap` alimenta `getMercadoReal()` (`lib/market-real.ts`) → `api/process-invoice` (el validador de facturas de indexados) — el validador llevaba meses comparando componentes de factura contra un SC/CAP mal calculado desde el origen.
...
**Pendiente:**
- No se ha revisado si alguna factura concreta fue validada (correcta o incorrectamente marcada) usando el SC/CAP incorrecto de abril-junio 2026 — si en ese periodo se cerró alguna gestión/reclamación de indexados apoyada en el validador, revisarla con el dato ya corregido.
