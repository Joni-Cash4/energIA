...
1. **Base de las tasas de Próxima mal compuesta.** El contrato dice *"Tasas, iguales al 1,5% de la suma de todos los conceptos anteriores, exceptuando los correspondientes a peajes y cargos"*. Verificado contra factura: la base es `Energía Mercado + FNEE + GO + bono social`, y **no** incluye el cargo de gestión (ago-26: 469,26+7,19+2,08+0,76 = 479,29 €, exactamente la base declarada; jul-26: 658,00 €, idem). `simIndexada` calcula `(mercadoPuro + feeTotal) × 0,015`, que mete la gestión y deja fuera FNEE/GO/bono. Error pequeño (+0,13 € en la factura de agosto) pero sistemático. **Resuelto el 2026-09-16** en [ADR-0013](0013-costes-proxima-contrastados-con-facturas.md), también en el Python.
2. **Origen del PERD.** El contrato de Próxima nombra los ficheros `PERDQHxx` / `PERDxx` de REE, `(1 + valor) × 1,04`. El [ADR-0006](0006-mercado-perd-desde-esios.md) usa `COF2TD` de PVPCDATA. **Aclarado a medias el 2026-09-16** ([ADR-0013](0013-costes-proxima-contrastados-con-facturas.md)): no coinciden, porque `COF2TD` es el perfil de consumo (≈0,0001 por hora), no pérdidas, y nuestro PERD es en la práctica un 1,04 fijo. En 5 facturas reales esa calibración da −1,3 % en total. Pendiente: probar con los ficheros `PERDQH` de REE, que son los que nombra el contrato. **Probados el mismo día** (ver ADR-0013, «Pérdidas reales de REE»): son de un 15 % y, con nuestro SC, el total sale un 9 % alto; se mantiene el 1,04 hasta tener una curva.
3. **`HORAS_PERIODO`** (`market-rates.ts`) está definido y no se usa en ningún sitio; además tiene P6 como solo las horas 0–7, sin fines de semana. Borrarlo o arreglarlo antes de que alguien lo use. **Borrado el 2026-09-16**, en la web y en el Python.
...
**Actualización 2026-09-16 — todo el Python pasa el CUPS:** hasta ahora solo el agente (`agente_carpetas.py`) pasaba el CUPS a `obtener_precios_mercado`, así que el resto de caminos del Python calculaba siempre con la media simple del periodo. Ahora también lo pasan la API local (`api/main.py`, sus tres cálculos de comparativa), `main.py`, `procesar_imagen.py` (que además no pasaba la tarifa: calculaba todo como 3.0TD) y las renovaciones (`modules/renovaciones.py`, con el CUPS del cliente). Comprobado con un CUPS con curva en agosto de 2026: sin CUPS, P3 103,52 y P4 99,99 €/MWh (`SUPABASE`); con CUPS, 95,22 y 94,37 (`SUPABASE_CURVA`). Sin curva, el resultado no cambia.
