...
**Actualización 2026-09-16 — aviso si faltan datos:**
- El 16/09 el cron diario de SC/CAP/PERD falló en silencio (ver [ADR-0006](0006-mercado-perd-desde-esios.md)) y solo se vio al revisarlo a mano. Los crons se ponen al día solos, pero mientras tanto las facturas de esos días usan el valor del mes o una estimación sin que nadie lo sepa.
- Nuevo cron `/api/cron/mercado-vigilancia` (7:30 UTC, después de los que rellenan las tablas). No llama a ESIOS: solo lee Supabase y comprueba los últimos 7 días hasta ayer (`mercado_pmd_diario` con 23-25 horas por día y `mercado_sc_cap_perd_diario` presente) y, desde el día 3, el mes anterior en `mercado_sc_cap` y `mercado_perd`. Si falta algo, avisa por Telegram al chat de `TELEGRAM_ALLOWED_CHAT_ID`; si está todo bien, no envía nada. Si no puede avisar, el cron responde con error.
- Probado en local contra Supabase real (con el envío a Telegram sustituido por una impresión): hoy sin avisos; simulando el 18/09, avisa de los días 16 y 17 en las dos tablas; simulando el 05/10, además del mes de septiembre en las mensuales.
