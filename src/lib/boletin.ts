...
// lo consume una página PÚBLICA, así que nunca debe usar ESIOS.
// IMPORTANTE: aquí NO hay datos mock — es contenido publicado; si REE falla,
// se lanza error y el consumidor decide (503 en la API, no enviar en el cron).
// Usado por /api/boletin (web) y /api/cron/boletin-semanal (email).
...
export interface BoletinDia { fecha: string; label: string; media: number | null; mediaAnterior: number | null }
export interface BoletinTecnologia { nombre: string; renovable: boolean; gwh: number; cuota: number; variacion: number | null }
export interface BoletinData {
  semana: { inicio: string; fin: string; label: string }
  precios: {
    dias: BoletinDia[]; media: number; mediaSemanaAnterior: number | null; variacionSemanal: number | null
  ...
  generacion: {
    ...
    variacionRenovables: number | null; tecnologias: BoletinTecnologia[]
  }
...
export function latestCompleteMonday(): Date {
...
export function fmtEs(n: number, dec = 1): string {
  return n.toLocaleString('es-ES', { minimumFractionDigits: dec, maximumFractionDigits: dec })
}

function signo(v: number | null): string {
  if (v == null) return ''
  return v >= 0 ? `+${fmtEs(v)}%` : `${fmtEs(v)}%`
}

// ── Construcción del boletín ────────────────────────────────────────────────

export async function buildBoletin(startParam?: string | null): Promise<BoletinData> {
  ...
  // ── Precios ──
  const dias: BoletinDia[] = Array.from({ length: 7 }, (_, i) => {
  ...
  if (valores.length < 5 || !gen.total || !dem) {
    throw new Error('Datos de REE incompletos para esta semana')
  }
  ...
  const tecnologias: BoletinTecnologia[] = gen.techs
  ...
  mercado.push(
    `El precio medio del mercado mayorista se situó en ${fmtEs(media, 2)} €/MWh esta semana` +
    (varSemanal != null ? `, un ${fmtEs(Math.abs(varSemanal))}% ${varSemanal >= 0 ? 'más' : 'menos'} que la semana anterior` : '') +
    (varInteranual != null ? ` y un ${fmtEs(Math.abs(varInteranual))}% ${varInteranual >= 0 ? 'por encima' : 'por debajo'} del mismo periodo de ${lyMonday.getUTCFullYear()}` : '') + '.'
  )
  if (diaMax.media != null && diaMin.media != null) {
    ...
    mercado.push(
      ...
      `(${fmtEs(diaMax.media, 2)} €/MWh) y el más barato el ${fmtDay(new Date(diaMin.fecha + 'T12:00:00Z'), { weekday: 'long', day: 'numeric' })} ` +
      `(${fmtEs(diaMin.media, 2)} €/MWh)` +
  ...
  if (varDemanda != null) {
    balance.push(
      `La demanda eléctrica peninsular fue de ${fmtEs(demGwh, 0)} GWh, un ${fmtEs(Math.abs(varDemanda))}% ` +
      `${varDemanda >= 0 ? 'superior' : 'inferior'} a la del mismo periodo del año pasado.`
    )
  }
  const varRen = pct(renMwh, renMwhLy)
  balance.push(
    `Las energías renovables aportaron el ${fmtEs(cuotaRen)}% de la generación peninsular` +
    (varRen != null ? ` (${signo(varRen)} interanual)` : '') +
    (top ? `. La tecnología con mayor peso fue ${top.nombre.toLowerCase()} con una cuota del ${fmtEs(top.cuota)}%` +
